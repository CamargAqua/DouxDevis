...
def _add_header(doc: Document, marque: str, maison_name: str | None = None) -> None:
    ...
    # Premier mot en grand, reste en doré — rendu identique à l'historique pour "DOUX Joaillier".
    name = (maison_name or "").strip() or "DEVIS"
    first, _, rest = name.partition(" ")
    _add_run(p, first.upper(), bold=True, size=44, color=RGBColor(0, 0, 0))
    sub = left.add_paragraph()
    sub.alignment = WD_ALIGN_PARAGRAPH.LEFT
    if rest:
        _add_run(sub, rest.upper(), bold=True, size=12, color=DOUX_GOLD)
...
def _add_photos_inline(paragraph, photos: list[bytes], width: Cm) -> None:
    """Insère 1 ou 2 photos côte à côte dans un même paragraphe (même case de tableau)."""
    for i, blob in enumerate(photos):
        if i > 0:
            paragraph.add_run(" ")
        run = paragraph.add_run()
        try:
            run.add_picture(BytesIO(blob), width=width, height=width)
        except Exception:
            _add_run(paragraph, "[photo]", italic=True, size=9)


def _add_montre_table(doc: Document, montre: dict[str, Any], photos: list[bytes],
                      marque: str = "") -> None:
    """Tableau montre redesigné : en-tête sombre (modèle) + corps (état | ref/série).

    photos : 0 à 2 photos de la pièce, affichées côte à côte dans la même case
    (pas de grille séparée pour cette section — contrainte de mise en page).
    """
    photos = (photos or [])[:2]
    ...
    # ── Cas sans info : ligne unique (avec les photos si présentes) ──────────
    if not has_info:
        tbl = doc.add_table(rows=1, cols=1)
        tbl.style = "Table Grid"
        cell = tbl.cell(0, 0)
        default = f"Pièce {marque.upper()}".strip() + (f" — {modele_raw}" if modele_raw else "")
        _add_run(cell.paragraphs[0], default, italic=True, size=10)
        if photos:
            pp = cell.add_paragraph()
            pp.alignment = WD_ALIGN_PARAGRAPH.CENTER
            _add_photos_inline(pp, photos, Cm(4) if len(photos) == 1 else Cm(2.1))
        return

    # ── Layout : 2 lignes × 2 cols (+ col photo optionnelle) ────────────────
    DARK_HEX = "1A1814"
    col_left  = Cm(12) if not photos else Cm(7.5)
    col_right = Cm(6)
    col_photo = Cm(4.5)
    ...
    # Photo(s) — 1 ou 2 côte à côte dans la même case (col 0, ligne unique)
    if photos:
        ph = tbl.cell(0, 0)
...
def _add_work_table(doc: Document, title: str, lines: list[dict[str, Any]],
    ...
    for i, line in enumerate(lines):
        ...
        photo = line.get("_photo_bytes")
        if photo:
            photo_p = desc_cell.add_paragraph()
            photo_p.paragraph_format.space_before = Pt(3)
            run = photo_p.add_run()
            try:
                run.add_picture(BytesIO(photo), width=Cm(2.2), height=Cm(2.2))
            except Exception:
                pass
...
_DOCX_FOOTER_DEFAULT = ("", "")


def _add_footer_block(doc: Document, delai: str, frais_refus: int | None = None, lieu: str = "",
                      maison_name: str | None = None, legal_footer: bool = False) -> None:
    p = doc.add_paragraph()
    ...
    doc.add_paragraph()
    # legal_footer : mentions légales réelles + emails SAV + CGV/QR, spécifiques Doux
    # (même logique que _footer_for de pdf_generator). Sinon : nom de la maison seul.
    if not legal_footer:
        legal = doc.add_paragraph()
        legal.alignment = WD_ALIGN_PARAGRAPH.CENTER
        _add_run(legal, maison_name or "", size=8)
        return
    _lieu_key = (lieu or "").strip().lower()
    footer_lines = _DOCX_FOOTERS.get(_lieu_key, _DOCX_FOOTER_DEFAULT)
    legal = doc.add_paragraph()
    legal.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    # Email SAV + CGV/QR : spécifiques aux boutiques Doux, pas d'équivalent pour les autres
    # tenants en legal_footer (ex. Lepage — pas d'email SAV connu, pas de CGV/QR à eux).
    if _lieu_key not in ("avignon", "nimes", "nîmes"):
        return
    if footer_lines[1]:
        legal2.add_run("  ")
...
def build_docx(data: dict[str, Any],
               maison_name: str | None = None, legal_footer: bool = False,
               photos: dict[str, list[bytes]] | None = None) -> bytes:
    """Construit le devis et renvoie le contenu binaire .docx.

    legal_footer=True : mentions légales/emails/CGV Doux (comme pdf_generator) ;
    sinon footer neutre avec maison_name.
    photos : {"montre": [0-2 photos, affichées dans la case infos]}.
    Les photos d'options sont portées par chaque ligne (`line["_photo_bytes"]`),
    pas par ce dict.
    """
    photos = photos or {}
    doc = Document()
    ...
    marque = marque if marque is not None else "PARTENAIRE"
    client = data.get("client") or {}
    sav = data.get("sav") or {}
    montre = data.get("montre") or {}

    _add_header(doc, marque, maison_name=maison_name)
    ...
    _add_run(titre, data.get("titre") or "DEVIS DE RÉPARATION", bold=True, size=14)
    ...
    # Phrase d'introduction (paramétrable, voir data["titre"] ci-dessus)
    _m = montre
    ...
    _add_montre_table(doc, montre, photos.get("montre") or [], marque=marque)
    ...
    _add_footer_block(doc, delai=data.get("delai") or "4 à 6 semaines", frais_refus=frais_refus,
                      ...
                      maison_name=maison_name, legal_footer=legal_footer)
