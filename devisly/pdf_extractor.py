...
class ExtractionError(RuntimeError):
    """Erreur d'extraction dont le message est destiné à l'utilisateur final (français, actionnable)."""

EXTRACTION_SYSTEM = """Tu es un assistant chargé d'extraire les informations d'un devis de service après-vente (horlogerie ou joaillerie) envoyé par une marque partenaire à une bijouterie ou horlogerie.
...
═══ LIEU ═══
Cherche dans le document la ville de la bijouterie/horlogerie destinataire de ce devis (le revendeur, pas la marque partenaire émettrice).
Elle apparaît dans des champs comme : "Point de vente", "Magasin", "Client", "Retailer", "Revendeur", adresse du destinataire.
Renvoie le nom de la ville tel qu'il apparaît (n'importe quelle ville, pas de liste fermée), avec une majuscule initiale (ex: "Lille", "Nîmes", "Lyon").
Si aucune ville n'est identifiable avec certitude → chaîne vide. Ne jamais deviner ou inventer une ville.
...
def _require_api_key(api_key: str | None) -> str:
    ...
    if not api_key:
        raise ExtractionError("Clé API d'extraction non configurée — contactez votre prestataire.")
    return api_key


def _call_claude(user_content: list[dict], api_key: str,
                 max_tokens: int = 2000, system: str = EXTRACTION_SYSTEM) -> str:
    """Appel unique à l'API Claude avec traduction des erreurs en messages actionnables.

    L'ordre des except respecte la hiérarchie du SDK (spécifique avant générique) :
    AuthenticationError/RateLimitError/BadRequestError héritent d'APIStatusError,
    APITimeoutError hérite d'APIConnectionError.
    """
    ...
    try:
        ...
        raise ExtractionError(
            "Clé API d'extraction invalide ou révoquée — contactez votre prestataire."
        ) from exc
    except anthropic.RateLimitError as exc:
        raise ExtractionError(
            "Le service d'extraction est saturé pour le moment. Réessayez dans une minute."
        ) from exc
    except anthropic.BadRequestError as exc:
        raise ExtractionError(
            "Le document a été refusé par le service d'extraction — fichier corrompu, "
            "protégé par mot de passe ou trop volumineux. Ré-exportez le PDF, "
            "ou utilisez l'onglet « Coller un email »."
        ) from exc
    except anthropic.APIStatusError as exc:
        if exc.status_code in (500, 502, 503, 529):
            raise ExtractionError(
                f"Le service d'extraction est temporairement indisponible (erreur {exc.status_code}). "
                "Réessayez dans quelques minutes."
            ) from exc
        raise ExtractionError(
            f"Erreur du service d'extraction (code {exc.status_code}). "
            "Réessayez — si le problème persiste, contactez votre prestataire."
        ) from exc
    except anthropic.APITimeoutError as exc:
        raise ExtractionError(
            "L'analyse a dépassé le délai maximum (60 s). Réessayez — si le problème persiste, "
            "le document est peut-être trop long ou trop complexe."
        ) from exc
    except anthropic.APIConnectionError as exc:
        raise ExtractionError(
            "Impossible de joindre le service d'extraction. Vérifiez la connexion internet "
            "du poste, puis réessayez."
        ) from exc
...
def extract_from_pdf(pdf_bytes: bytes, api_key: str | None = None,
    ...
    raw = _call_claude(user_content, api_key)
...
def _parse_claude_response(raw: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ExtractionError(
            "L'analyse n'a pas produit un résultat exploitable. Réessayez — "
...
def _extract_from_text(text: str, api_key: str | None = None,
    ...
    return _clean(_parse_claude_response(_call_claude(user_content, api_key)))
...
def extract_from_eml(eml_bytes: bytes, api_key: str | None = None,
    ...
    if not body.strip():
        raise ExtractionError(
            "Cet email ne contient ni PDF joint ni texte lisible. "
            "Vérifiez le fichier, ou copiez-collez le contenu du mail."
        )
...
_MULTI_DEVIS_SYSTEM = """Tu analyses un PDF envoyé par une marque partenaire à une bijouterie.
Il peut contenir UN SEUL devis SAV, ou PLUSIEURS devis SAV distincts concaténés (clients différents, numéros SAV différents).

Renvoie UNIQUEMENT un objet JSON (sans texte autour, sans markdown) :
{"count": <nombre de devis distincts>, "ranges": [[page_debut, page_fin], ...]}

- Les pages sont numérotées à partir de 1, bornes incluses.
- Un devis = un client + un numéro SAV. Des pages annexes (CGV, conditions) se rattachent au devis qui les précède.
- S'il n'y a qu'un seul devis : {"count": 1, "ranges": [[1, <dernière page>]]}"""


def detect_multi_devis(pdf_bytes: bytes, api_key: str | None = None) -> list[tuple[int, int]]:
    """Détecte les plages de pages des devis distincts d'un PDF. [(debut, fin)] 1-indexé.

    Retourne [] si un seul devis (ou en cas de doute) — l'appelant traite alors le PDF entier.
    """
    api_key = api_key or os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        return []
    try:
        raw = _call_claude(
            ...
            api_key, max_tokens=300, system=_MULTI_DEVIS_SYSTEM,
        )
        result = _parse_claude_response(raw)
        count = int(result.get("count") or 1)
        ranges = [(int(a), int(b)) for a, b in (result.get("ranges") or [])]
        if count <= 1 or len(ranges) <= 1:
            return []
        return ranges
    except Exception as exc:
        # En cas de doute : traiter comme un devis unique (fail-safe), mais tracer.
        logger.warning("Détection multi-devis échouée, PDF traité entier : %s", exc)
        return []


def split_pdf_pages(pdf_bytes: bytes, ranges: list[tuple[int, int]]) -> list[bytes]:
    """Découpe un PDF en sous-PDF selon des plages de pages 1-indexées incluses."""
...
def extract_from_msg(msg_bytes: bytes, api_key: str | None = None,
    ...
    if not body.strip():
        raise ExtractionError(
            "Ce message Outlook ne contient ni PDF joint ni texte lisible. "
            "Vérifiez le fichier, ou copiez-collez le contenu du mail."
        )
