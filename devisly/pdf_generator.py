...
def _logo_img(path: Path | bytes, max_w: float, max_h: float, width_first: bool = False) -> Image:
    """Charge un logo (chemin fichier ou bytes), rogne les bordures vides, et scale."""
    from io import BytesIO as _BIO
    _is_bytes = isinstance(path, (bytes, bytearray))
    src: str | _BIO = _BIO(path) if _is_bytes else str(path)
    try:
        from PIL import Image as _PIL, ImageOps as _IOps
        pil = _PIL.open(_BIO(path) if _is_bytes else str(path)).convert("RGBA")
        r, g, b, a = pil.split()
...
_FOOTERS: dict[str, str] = {
    ...
    "lille": (
        "SARL BIJOUTERIE LEPAGE LILLE au Capital de 1 223 175 € — R.C.S. Lille Métropole 600 500 920 "
        "— Siret 600 500 920 00050 — APE 4777 Z — TVA Intracommunautaire FR51 600 500 920 "
        "— 6-8-10 RUE DE LA BOURSE, 59800 LILLE"
    ),
    "thionville": (
        "SAS DANIEL GERARD - JOAILLIER au Capital de 12 300 € — R.C.S. Thionville 453 763 278 "
        "— Siret 453 763 278 00018 — APE 4777 Z — TVA Intracommunautaire FR91 453 763 278 "
        "— 8 RUE DE LUXEMBOURG, 57100 THIONVILLE"
    ),
}


def _footer_for(lieu: str, maison_name: str | None = None, legal_footer: bool = False) -> str:
    # legal_footer : mentions légales réelles keyed par lieu (villes connues uniquement :
    # Avignon/Nîmes/Lille) — ville inconnue ou absente → rien plutôt qu'une mention fausse.
    # Sinon : pied de page générique au nom de la maison du tenant.
    if legal_footer:
        return _FOOTERS.get((lieu or "").strip().lower(), "")
    return maison_name or ""
...
def _make_draw_footer(lieu: str, maison_name: str | None = None, legal_footer: bool = False):
    footer_text = _footer_for(lieu, maison_name, legal_footer)
    def _draw_footer(canvas, doc):
        ...
        # Ligne mentions légales / nom de la maison
        p = Paragraph(footer_text, base_style)
        p.wrap(text_w, 20 * mm)
        p.drawOn(canvas, 15 * mm, 10 * mm)
        # Phrase CGV + QR code : spécifiques aux boutiques Doux (QR pointe vers les CGV Doux) —
        # ne pas afficher pour les autres tenants en legal_footer (ex. Lepage, pas de CGV/QR à eux).
        # ponytail: CGV/QR par tenant si un client le demande — pas de mécanisme générique tant que non requis.
        if legal_footer and (lieu or "").strip().lower() in ("avignon", "nimes", "nîmes"):
...
def _montre_photo_cell(photos: list[bytes]) -> Any | None:
    """1 ou 2 photos de la pièce côte à côte dans la même case (pas de grille séparée)."""
    photos = (photos or [])[:2]
    size = 4 * cm if len(photos) == 1 else 2.1 * cm
    imgs: list[Any] = []
    for blob in photos:
        try:
            img = Image(BytesIO(blob))
            img.drawWidth = img.drawHeight = size
            imgs.append(img)
        except Exception:
            continue
    if not imgs:
    ...
    if len(imgs) == 1:
        return imgs[0]
    inner = Table([imgs], colWidths=[size] * len(imgs))
...
def render_pdf(data: dict[str, Any],
               maison_name: str | None = None, logo_bytes: bytes | None = None,
               legal_footer: bool = False,
               photos: dict[str, list[bytes]] | None = None) -> bytes:
    photos = photos or {}
    buf = BytesIO()
    ...
    # ── En-tête ───────────────────────────────────────────────────────────────
    marque_raw = data.get("marque")
    marque_raw = marque_raw if marque_raw is not None else "PARTENAIRE"
    marque_up  = marque_raw.upper()

    # Logo maison (gauche) : logo du tenant (bytes DB) sinon nom de la maison en texte.
    _LOGO_MAX_W   = 8 * cm
    _DOUX_MAX_H   = 1.2 * cm  # taille du logo maison (tous tenants, pas que Doux malgré le nom)
    _BRAND_MAX_H  = 0.8 * cm

    if logo_bytes:
        left_cell = _logo_img(logo_bytes, _LOGO_MAX_W, _DOUX_MAX_H)
        left_cell.hAlign = 'LEFT'
    else:
        _maison = (maison_name or "MAISON").upper()
    ...
    # ── Titre (paramétrable : "DEVIS DE RÉPARATION" par défaut, ex. "DEVIS" pour une création) ──
    titre_style = ParagraphStyle(
        "titre_devis", fontName="Helvetica-Bold", fontSize=13,
        alignment=1, leading=18, textColor=DARK,
    )
    story.append(_p(data.get("titre") or "DEVIS DE RÉPARATION", titre_style))
    story.append(Spacer(1, 3 * mm))
    ...
    montre_photo_cell = _montre_photo_cell(photos.get("montre") or [])
    if montre_photo_cell:
        montre_tbl = Table([[
            montre_photo_cell,
        ...
        ts.add("BACKGROUND", (0, 0), (0, 0), colors.white)
    ...
    def _photo_cell(base_cell, photo_bytes, base_w, photo_w):
        """Insère une vignette 2,2cm à côté d'une cellule si une photo est attachée à la
        ligne — factorisé entre nécessaire et optionnel (seule l'optionnelle l'avait avant)."""
        if not photo_bytes:
            return base_cell
        try:
            thumb = Image(BytesIO(photo_bytes))
            thumb.drawWidth = thumb.drawHeight = 2.2 * cm
            inner = Table([[base_cell, thumb]], colWidths=[base_w, photo_w])
            inner.setStyle(TableStyle([
                ("VALIGN",        (0, 0), (-1, -1), "MIDDLE"),
    ...
    for i, line in enumerate(necessaires):
        ...
        if i == 0 and intro:
            ...
            # Détail en plus petit sous la description — ex. le détail des opérations d'une
            # famille regroupées en une phrase par l'horloger/Haiku (cf. app.py _haiku_polish).
            cell_desc = _html(f'<b>{desc}</b><br/><font size="8">{detail}</font>', base)
        else:
            cell_desc = _p(desc, base)
        cell_desc = _photo_cell(cell_desc, line.get("_photo_bytes"), 11 * cm, 3 * cm)
    ...
    if optionnelles:
        ...
        for i, l in enumerate(optionnelles):
            ...
            checkbox = _CheckboxField(f"option_{i}", desc, font="Helvetica-Bold", font_size=9)
            desc_cell: Any = _photo_cell(checkbox, l.get("_photo_bytes"), 11 * cm, 3 * cm)
            opt_rows.append([
                desc_cell,
    ...
    lieu = (data.get("sav") or {}).get("lieu") or ""
    _footer_fn = _make_draw_footer(lieu, maison_name, legal_footer)
...
def docx_to_pdf(docx_bytes: bytes, data: dict[str, Any] | None = None,
                maison_name: str | None = None,
                logo_bytes: bytes | None = None, legal_footer: bool = False,
                photos: dict[str, list[bytes]] | None = None) -> tuple[bytes, str]:
    if data is None:
        raise RuntimeError("Données requises pour la génération PDF.")
    return render_pdf(data, maison_name=maison_name,
                      logo_bytes=logo_bytes, legal_footer=legal_footer,
                      photos=photos), "reportlab"
